...
/**
 *      deriza o vetor 'dispositivos' na tabela HTML.
...
async function excluirDispositivo() {
  // 1. Ler o ID do campo e validar
  const id = campoId.value.trim();

  if (!id) {
    mostrarMensagem('Digite o ID do dispositivo que deseja excluir.', 'erro');
    return;
  }

  // 2. Confirmar com o usuário antes de excluir
  const confirmou = confirm(`Tem certeza que deseja excluir o dispositivo com ID ${id}?`);
  if (!confirmou) {
    return; // Usuário cancelou a exclusão
  }

  try {
    // 3. Fazer a requisição DELETE
    const respostaHTTP = await fetch(`${URL_API}/${id}`, {
      method: 'DELETE'
    });
    ...
    if (!respostaHTTP.ok) {
      mostrarMensagem(`Erro ao excluir. Dispositivo com ID ${id} não encontrado ou erro na API.`, 'erro');
      return;
    }

    // 5. Remover o item do vetor local usando filter()
    dispositivos = dispositivos.filter(d => d.id !== id);

    // 6. Atualizar a tabela
    renderizar();

    // 7. Limpar o formulário e mostrar mensagem de sucesso
    limparFormulario();
    mostrarMensagem(`Dispositivo com ID ${id} excluído com sucesso!`, 'sucesso');
